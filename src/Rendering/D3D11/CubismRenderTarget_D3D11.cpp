...
#include "CubismRenderTarget_D3D11.hpp"
...
void CubismRenderTarget_D3D11::CopyBuffer(ID3D11DeviceContext* renderContext, CubismRenderTarget_D3D11& src, CubismRenderTarget_D3D11& dst)
{
    renderContext->CopyResource(dst._texture, src._texture);
}

CubismRenderTarget_D3D11::CubismRenderTarget_D3D11()
    : _texture(NULL)
    , _textureView(NULL)
...
void CubismRenderTarget_D3D11::BeginDraw(ID3D11DeviceContext* renderContext)
{
...
void CubismRenderTarget_D3D11::EndDraw(ID3D11DeviceContext* renderContext)
{
    if (_textureView == NULL || _renderTargetView == NULL || _depthView == NULL)
    {
        return;
...
void CubismRenderTarget_D3D11::Clear(ID3D11DeviceContext* renderContext, float r, float g, float b, float a)
{
...
csmBool CubismRenderTarget_D3D11::CreateRenderTarget(ID3D11Device* device, csmUint32 displayBufferWidth, csmUint32 displayBufferHeight)
{
    // 一旦削除
    DestroyRenderTarget();
...
void CubismRenderTarget_D3D11::DestroyRenderTarget()
{
...
ID3D11ShaderResourceView* CubismRenderTarget_D3D11::GetTextureView() const
{
    return _textureView;
}

csmUint32 CubismRenderTarget_D3D11::GetBufferWidth() const
{
    return _bufferWidth;
}

csmUint32 CubismRenderTarget_D3D11::GetBufferHeight() const
{
    return _bufferHeight;
}

csmBool CubismRenderTarget_D3D11::IsValid() const
{
    if (_textureView == NULL || _renderTargetView == NULL || _depthView == NULL)
    {
        return false;
